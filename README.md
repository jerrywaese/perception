...
Since it involves some graphic images that will be perceived by the script and since it uses canvas, the cross site limitations require it be served over the network using https:// as in (https://jerrywaese.github.io/perception/)
...
The model does not include the inter column and intra column high speed suppression of unchanging activation patterns which drive current active mental content but it shows how memory (or familiarity) is formed by interconnecting currently active cortical neurons, and it shows that the same mechanism produces a percitive reflex at the same time - reactivating resting neurons that have the ARC protein spines for the same interconnections 
